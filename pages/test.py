...
def main():
    ...
    # Only two types of reports for selection
    col1, col2 = st.columns(2)
    with col1:
        noon_at_sea = st.checkbox("Noon at Sea")
    with col2:
        noon_report = st.checkbox("Noon")

    # Update report_type based on selection
    if noon_at_sea:
        st.session_state.report_type = "Noon at Sea"
    elif noon_report:
        st.session_state.report_type = "Noon"
    else:
        st.session_state.report_type = None
    ...
    elif st.session_state.report_type == "Noon":
        st.markdown("### Noon Report")
        display_custom_report_form("Noon")
...
def display_default_section(section):
    st.write(f"Default display for {section}")
    # Add any default fields or information here
...
def display_speed_position_and_navigation():
    ...
    with col3:
        st.time_input("Date Time (UTC)", value=datetime.now().time(), key=f"utc_time_{uuid.uuid4()}")
        st.text("Latitude")
...
def display_custom_speed_position_and_navigation(noon_report_type):
    display_speed_position_and_navigation()  # Reuse the base function for custom report
...
def display_custom_weather_and_sea_conditions(noon_report_type):
    display_weather_and_sea_conditions()  # Reuse the base function for custom report
...
def display_custom_cargo_and_stability(noon_report_type):
    display_cargo_and_stability()  # Reuse the base function for custom report
...
def display_fuel_consumption():
    ...
    if 'density' not in st.session_state:
        st.session_state.density = {tank: np.random.uniform(0.8, 1.0) for tank in st.session_state.tanks}
    if 'bdn_number' not in st.session_state:
        st.session_state.bdn_number = {tank: f"BDN-{np.random.randint(1000, 9999)}" for tank in st.session_state.tanks}
    ...
    def create_editable_dataframe():
        index = ['Tank Properties', 'Previous ROB'] + st.session_state.consumers
        if bunker_survey:
            index.append('Bunker Survey Correction')
        index.append('Current ROB')
        df = pd.DataFrame(index=index, columns=st.session_state.tanks)
        
        # Add Tank Properties row
        tank_properties = [f"Visc: {st.session_state.viscosity[tank]:.1f}\nSulfur: {st.session_state.sulfur[tank]:.2f}%\nDensity: {st.session_state.density[tank]:.3f}\nBDN: {st.session_state.bdn_number[tank]}" for tank in st.session_state.tanks]
        df.loc['Tank Properties'] = tank_properties
    ...
    df_html = df_html.replace('<td>Tank Properties</td>', '<td style="font-weight: bold;">Tank Properties</td>')
    for tank in st.session_state.tanks:
        df_html = df_html.replace(f'<td>{df.loc["Tank Properties", tank]}</td>', f'<td class="tank-properties">{df.loc["Tank Properties", tank]}</td>')
    ...
    def edit_tank_properties():
        ...
        tank_props = pd.DataFrame({
            ...
            'BDN Number': st.session_state.bdn_number
        })
        edited_props = st.data_editor(
            ...
            column_config={
                ...
                'Density': st.column_config.NumberColumn(
                    'Density', min_value=0.8, max_value=1.0, step=0.001, format="%.3f"
                ),
        ...
        st.session_state.density = edited_props['Density'].to_dict()
        st.session_state.bdn_number = edited_props['BDN Number'].to_dict()
...
def display_custom_machinery(noon_report_type):
    display_machinery()  # Reuse the base function for custom report
...
def display_custom_miscellaneous_consumables(noon_report_type):
    display_miscellaneous_consumables()  # Reuse the base function for custom report
