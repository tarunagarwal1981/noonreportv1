...
def display_voyage_manifest():
    st.write(f"Voyage ID: {st.session_state.current_voyage['id']}")
    st.write(f"Current Voyage Status: {st.session_state.current_voyage['status']}")

    col1, col2, col3, col4, col5 = st.columns(5)
    with col1:
        if st.button("Start New Voyage Manifest"):
            if st.session_state.current_voyage['status'] == 'Closed':
                create_new_voyage()
                st.success(f"New Voyage Manifest started in Draft mode! Voyage ID: {st.session_state.current_voyage['id']}")
                st.experimental_rerun()
            else:
    ...
    with col3:
        if st.button("Open Voyage"):
            if st.session_state.current_voyage['status'] == 'Draft':
                st.session_state.current_voyage['status'] = 'Open'
                st.success("Voyage opened successfully!")
            else:
                st.error("Can only open a voyage in Draft status.")
    with col4:
        if st.button("Close Voyage"):
            if st.session_state.current_voyage['status'] == 'Open':
                st.session_state.current_voyage['status'] = 'Closed'
...
def general_info(edit_mode):
    ...
    with col4:
        ...
        voyage['general_info']['revision_date'] = st.date_input("Revision Date", value=voyage['general_info'].get('revision_date', None), disabled=not edit_mode or voyage['status'] == 'Closed')
...
def voyage_itinerary(edit_mode):
    ...
    edited_df = st.data_editor(voyage['itinerary'], disabled=not edit_mode or voyage['status'] == 'Closed', num_rows="dynamic")
...
def log(edit_mode):
    ...
    col1, col2, col3, col4 = st.columns(4)
    with col1:
        voyage['log']['created_by'] = st.text_input("Created By", value=voyage['log'].get('created_by', ''), disabled=True)
    with col2:
        voyage['log']['created_date'] = st.date_input("Created Date", value=voyage['log'].get('created_date', datetime.date.today()), disabled=True)
    with col3:
        voyage['log']['last_modified_by'] = st.text_input("Last Modified by", value=voyage['log'].get('last_modified_by', ''), disabled=True)
    with col4:
        voyage['log']['last_modified_datetime'] = st.text_input("Last Modified Datetime", value=voyage['log'].get('last_modified_datetime', datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")), disabled=True)

def display_past_voyages():
    st.subheader("Past Voyages")
    for voyage in st.session_state.voyages[:-1]:  # Exclude the current voyage
        st.write(f"Voyage ID: {voyage['id']}, Status: {voyage['status']}")
