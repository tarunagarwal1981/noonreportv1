...
<style>
    ...
    .stNumberInput, .stTextInput, .stSelectbox { padding-bottom: 0.5rem !important; }
    .stNumberInput input, .stTextInput input, .stSelectbox select { padding: 0.3rem !important; font-size: 0.9em !important; }
    .stExpander { border: none !important; box-shadow: none !important; margin-bottom: 0.5rem !important; }
    .history-panel { background-color: #f1f1f1; padding: 10px; border-radius: 5px; margin-bottom: 20px; max-width: 300px; }
    .history-panel h3 { margin-top: 0; margin-bottom: 10px; }
    .history-select { margin-bottom: 5px; }
    .field-prompt { font-size: 0.8em; color: #666; margin-bottom: 2px; }
    .small-warning { font-size: 8px; color: #b20000; background-color: #ffe5e5; border-radius: 5px; padding: 5px; }
    .info-message { font-size: 12px; color: #0066cc; background-color: #e6f2ff; padding: 5px; border-radius: 3px; margin-top: 5px; margin-bottom: 10px; display: inline-block; }
</style>
            ...
            if field == "Vessel Name":
                ...
                if field == "ME Other (mt)" and not me_fields_processed:
                    if me_total_consumption > 25:
                        st.markdown('<p class="small-warning">Total ME consumption exceeds expected consumption of 25.</p>', unsafe_allow_html=True)
                ...
                if field == "AE Other (mt)" and not ae_fields_processed:
                    if ae_total_consumption > 3:
                        st.markdown('<p class="small-warning">Total AE consumption exceeds expected consumption of 3.</p>', unsafe_allow_html=True)
                    st.markdown('<p class="info-message">MFM figures since last report</p>', unsafe_allow_html=True)
                ...
                if me_total_consumption > 15 and not boiler_message_shown:
                    st.markdown('<p class="small-warning">Since Main Engine is running at more than 50% load, Boiler consumption is expected to be zero.</p>', unsafe_allow_html=True)
...
# Custom CSS for compact layout, history panel, field prompts, and scrollable boxes
st.markdown("""
<style>
    ...
    for i, field in enumerate(fields):
        with cols[i % 4]:  # This will cycle through the columns
            ...
            if field == "Vessel Name":
                ...
                # Display AIS position message after all position fields have been processed
                if position_fields_processed == 6:
                    st.markdown('<p class="info-message">Current AIS position pre-filled based on random data. Edit if necessary.</p>', unsafe_allow_html=True)
                ...
                if field == "ME Other (mt)" and not me_fields_processed:
                    if me_total_consumption > 25:
                        st.markdown('<p class="info-message">Total ME consumption exceeds expected consumption of 25 mt.</p>', unsafe_allow_html=True)
                    st.markdown('<p class="info-message">MFM figures since last report</p>', unsafe_allow_html=True)
                    me_fields_processed = True
            elif field in ["AE LFO (mt)", "AE MGO (mt)", "AE LNG (mt)", "AE Other (mt)"]:
                if field == "AE LFO (mt)":
                ...
                if field == "AE Other (mt)" and not ae_fields_processed:
                    if ae_total_consumption > 3:
                        st.markdown('<p class="info-message">Total AE consumption exceeds expected consumption of 3 mt.</p>', unsafe_allow_html=True)
                    st.markdown('<p class="info-message">MFM figures since last report</p>', unsafe_allow_html=True)
                    ae_fields_processed = True
            elif field.startswith("Boiler"):
                value = st.number_input(field, min_value=0.0, max_value=4.0, step=0.1, key=field_key)
...
def main():
    ...
    with col1:
        st.markdown('<div class="reportSection scrollable-box">', unsafe_allow_html=True)
        if 'current_report_type' in st.session_state:
            create_form(st.session_state.current_report_type)
        else:
            st.write("Please use the AI Assistant to initiate a report.")
