...
def display_fuel_type_view(bunker_survey):
    ...
    column_config = {
        fuel: st.column_config.NumberColumn(
            fuel,
            help=f"Viscosity: {st.session_state.viscosity.get(fuel, 0):.1f}, Sulfur: {st.session_state.sulfur.get(fuel, 0):.2f}%",
            min_value=0.0,
            format="%.2f"
        ) for fuel in st.session_state.fuel_types if fuel != 'Other Fuel Type'
    }
    column_config['Other Fuel Type'] = st.column_config.TextColumn(
        "Other Fuel Type",
...
def display_bdn_view(bunker_survey):
    ...
    column_config = {
        tank: st.column_config.NumberColumn(
            tank,
            help=f"Viscosity: {st.session_state.viscosity.get(tank, 0):.1f}, Sulfur: {st.session_state.sulfur.get(tank, 0):.2f}%",
            min_value=0.0,
            format="%.2f"
