...
with tabs[1]:
    ...
    cargo_operation_qty = st.number_input("Cargo - Qty (MT)", min_value=0.0, step=0.01)
    cargo_operation_vapour_qty = st.number_input("Cargo - Vapour Qty (MT)", min_value=0.0, step=0.01)
    cargo_operation_oil_major_cargo = st.text_input("Cargo - Oil Major Cargo")
    cargo_operation_oil_major = st.text_input("Cargo - Oil Major")
    cargo_operation_basis_final_qty = st.text_input("Cargo - Basis of Final Qty")
    cargo_operation_btb_transfer = st.text_input("Cargo - BTB Transfer Y/N")
    cargo_operation_commenced_date = st.date_input("Cargo - Commenced Date", datetime.now().date())
    cargo_operation_commenced_time = st.time_input("Cargo - Commenced Time", datetime.now().time())
    cargo_operation_completed_date = st.date_input("Cargo - Completed Date", datetime.now().date())
    cargo_operation_completed_time = st.time_input("Cargo - Completed Time", datetime.now().time())
    cargo_operation_action = st.text_input("Cargo - Action")
